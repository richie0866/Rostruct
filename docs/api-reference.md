...
<span class="code">
	...
	&nbsp;&nbsp;&nbsp;&nbsp;<span class="symbol">=> Resolves once all scripts finish executing from [<span class="keyword"><u>Deploy</u></span>](#deploy).</span>
</span>
...
<span class="code">
	...
	&nbsp;&nbsp;&nbsp;&nbsp;<span class="symbol">=> Resolves with the value of the [<span class="keyword"><u>Require</u></span>](#require) operation.</span>  
	<span class="symbol">}</span>
</span>

Stores the value of a successful build operation performed by [`Build`](#build), [`Deploy`](#deploy), or [`Require`](#require).
...
Stores information about the release being downloaded by [`DownloadRelease`](#downloadrelease) or [`DownloadLatestRelease`](#downloadlatestrelease).
...
### <span class="code">Build</span> <span class="tag-space"></span> <span class="base-tag static-tag"></span>
...
<span class="code">
	...
	[<span class="interface"><u>BuildResult</u></span>](#buildresult)
</span>

Transforms files found at `location` into Roblox instances. This process is almost identical to to syncing Rojo to Roblox Studio, with [certain features omitted](guide/file-conversion.md).
...
| Name                               | Type                               | Required |
| ---------------------------------- | ---------------------------------- | -------- |
| <span class="code">location</span> | <span class="type">string</span>   | ✔        |
| <span class="code">parent</span>   | <span class="type">Instance</span> | ✘        |
...
### <span class="code">Deploy</span> <span class="tag-space"></span> <span class="base-tag static-tag"></span>
...
Runs every LocalScript object in the Instance created by [`Rostruct.Build`](#build). It adds an additional field, `RuntimeWorker`, to the [`BuildResult`](#buildresult) object.
...
| Type                                                              | Note                       |
| ----------------------------------------------------------------- | -------------------------- |
| [<span class="interface"><u>BuildResult</u></span>](#buildresult) | Adds field `RuntimeWorker` |


### <span class="code">Require</span> <span class="tag-space"></span> <span class="base-tag static-tag"></span>
...
If the project is a **module**, it executes it and saves the result to an additional field, `Module`, in the [`BuildResult`](#buildresult) object.

`Module` is a Promise which resolves with exactly what the root ModuleScript returned.

!!! warning
	Calling `Rostruct.Require()` on a directory with no `init.lua` file in it will throw an error. To require a project, the project folder must be set up to transform into a ModuleScript. See the [execution model page](guide/execution-model.md#requiring-a-library) for how to do this, and [Rojo's sync details](https://rojo.space/docs/6.x/sync-details/#scripts) for a detailed explanation.
...
| Type                                                              | Note                                    |
| ----------------------------------------------------------------- | --------------------------------------- |
| [<span class="interface"><u>BuildResult</u></span>](#buildresult) | Adds field `RuntimeWorker` and `Module` |


### <span class="code">DownloadRelease</span> <span class="tag-space"></span> <span class="base-tag static-tag"></span>
...
<span class="code">
	...
	[<span class="interface"><u>DownloadResult</u></span>](#downloadresult)
</span>

Downloads and saves a Github release of the `repo` owned by `owner`. The `tag` parameter refers to **the name of the tag**, but the tag name is typically the name of the release. If `asset` is not specified, it will download the source code for the release.

When downloading a release, it will be saved to a folder in the cache. All attempts to download a cached release will make no HTTP requests, and the function will not try to redownload it. The location of the cached folder is returned in the [`DownloadResult`](#downloadresult) object.
...
if downloadResult.Updated then
	print("Roact v1.4.0 installed!")
end

-- Requires Roact:
local Roact = Rostruct.Require(downloadResult.Location .. "src/").Module:expect()
```
...
| Name                            | Type                             | Required |
| ------------------------------- | -------------------------------- | -------- |
| <span class="code">owner</span> | <span class="type">string</span> | ✔        |
| <span class="code">repo</span>  | <span class="type">string</span> | ✔        |
| <span class="code">tag</span>   | <span class="type">string</span> | ✔        |
| <span class="code">asset</span> | <span class="type">string</span> | ✘        |

**Returns**

| Type                                                                                                                                            |
| ----------------------------------------------------------------------------------------------------------------------------------------------- |
| <span class="code"><span class="interface">Promise</span>&lt;[<span class="interface"><u>DownloadResult</u></span>](#downloadresult)&gt;</span> |


### <span class="code">DownloadLatestRelease</span> <span class="tag-space"></span> <span class="base-tag static-tag"></span>
...
If a new release is available, it will clear the cached data and download the new release. The [`DownloadResult`](#downloadresult) promise is only resolved *after* this operation completes.
...
-- and waits for the Promise to finish:
local downloadResult = Rostruct.DownloadLatestRelease("Roblox", "roact"):expect()

-- Output something if a new release was downloaded:
if downloadResult.Updated then
	print("Roact has been updated to " .. downloadResult.Tag .. "!")
end
...
### <span class="code">ClearReleaseCache</span> <span class="tag-space"></span> <span class="base-tag static-tag"></span> <span class="base-tag debug-tag"></span>
